...
InstructionSet = {
    ...
    0xfb: ['+invoke-super-quick/range','i3rc'],
    0xfc: ['+iput-object-volatile','t22c'],
    0xfd: ['+sget-object-volatile','s21c'],
    0xfe: ['+sput-object-volatile','s21c'],
}
...
class DexDecompiler:
    ...
    def __init__(self):
        ...
        #self.OneWordInstructions.extend(range(0xe3,0x100))   #全部都是优化后的opcode
        ...
        self.TwoWordInstructions.extend(range(0xf2,0xf8))
        ...
        self.ThreeWordInstructions.extend([0xf0,0xf8,0xf9,0xfa,0xfb])
    ...
    def decode_operation(self, op):
        if type(op) == str:
            iop = ord(op[0])
            if iop in InstructionSet.keys():
                method = getattr(self, 'decode_'+InstructionSet[iop][1]+'_args')
                if not method:
                    raise Exception("Method not implemented")
                args = method(op)
                return InstructionSet[iop][0] + args
            else:
                return '<unused>'
        elif type(op) == dict:
            return op
...
test = "\x12\x10\x22\x01\x02\x03\x71\x10\x32\x14\x00\x00\x0c\x02\x70\x20\x36\x14\x21\x00\xf8\x10\x07\x00\x01\x00\x0c\x01\x11\x01"
